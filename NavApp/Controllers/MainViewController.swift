...
extension MainViewController: UITextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if let venuePlace = mainView.secondTextField.text {
            venueString = venuePlace
            LocationService.getCoordinate(addressString: venuePlace) { (coordinate, error) in
                if let error = error {
                    print(error)
                } else {
                    VenueApiClient.getVenues(keyword: venuePlace, lattitude: coordinate.latitude, longitude: coordinate.longitude, date: "20190219", completionHandler: { (error, venues) in
                        if let error = error {
                            print(error)
                        } else if let venues = venues {
                            self.venues = venues                       }

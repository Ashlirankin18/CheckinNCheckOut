...
final class VenueApiClient {
  ...
  static func getVenues(lattitude:String,longitude:String,date:String,completionHandler: @escaping(AppError?,[Venue]?) -> Void ){
  ...
  static func getVenueInformation(venueId:String,date:String,completionHandler: @escaping (AppError?,AVenue?) -> Void){
  let urlString = "https://api.foursquare.com/v2/venues/\(venueId)?&client_id=\(SecretKeys.clientId)&client_secret=\(SecretKeys.clientKey)&v=\(date)"
    NetworkHelper.shared.performDataTask(endpointURLString: urlString, httpMethod: "GET", httpBody: nil) { (error, data) in
      ...
      if let data = data {
        do{
          let venueInfo = try JSONDecoder().decode(AVenue.self, from: data)
